...
contains
  ...
  subroutine handle_define(line)
    ...
    if (paren_start > 0) then
      ...
      print *, "Raw value before allocation: '", value, "', length = ", len(value)
      ...
      allocate(character(len=len_trim(value)) :: macros(num_macros)%value)
      ...
      if (index(temp, '...') > 0) then
        ...
        print *, "Defined variadic macro: ", trim(name), "(", (trim(macros(num_macros)%params(i))//", ", i=1,param_count), "...) = ", trim(value)
      else
  ...
  character(len=*), intent(in) :: line
  character(len=:), allocatable :: expanded, args_str, temp, va_args
      ...
      if (pos > 0) then
        ...
        if (macros(i)%num_params > 0 .or. macros(i)%is_variadic) then
          if (start <= len(expanded) .and. expanded(start:start) == '(') then
            ...
            print *, "Initial temp (raw): '", temp, "', length = ", len(temp), ", trimmed length = ", len_trim(temp)
            ...
            print *, "Initial temp (trimmed): '", trim(temp), "'"
            ...
            if (macros(i)%is_variadic) then
              ...
              do while (pos > 0)
                ...
                if (pos > 0) then
                  start = pos + 10
                  if (temp(start:start) == '_' .and. temp(start+1:start+1) == ')') then
                    temp = temp(:pos-1) // trim(va_args) // ')'
                  else
                    temp = temp(:pos-1) // trim(va_args) // temp(start:)
                  end if
