...
contains
  ...
  subroutine handle_define(line)
    ...
    character(len=MAX_LINE_LEN) :: name, value
    integer :: pos
    type(macro_t), allocatable :: temp(:)

    pos = index(line, ' ')
    name = trim(adjustl(line(pos+1:)))
    pos = index(name, ' ')
    if (pos > 0) then
      value = trim(adjustl(name(pos+1:)))
      name = trim(name(:pos-1))
    else
      value = ''
    end if
    ...
    if (num_macros > size(macros)) then
      allocate(temp(num_macros))
      temp(1:size(macros)) = macros
      call move_alloc(temp, macros)
    end if
    macros(num_macros)%name = trim(name)
    macros(num_macros)%value = trim(value)
  ...
  function expand_macros(line) result(expanded)
    character(len=*), intent(in) :: line
    ...
    do i = 1, num_macros
      ...
      do while (pos > 0)
        ...
        if (pos > 0) then
          ...
          expanded = expanded(:pos-1) // trim(macros(i)%value) // expanded(start:)
        end if
      end do
    end do
